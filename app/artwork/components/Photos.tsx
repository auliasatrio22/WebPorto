...
const Photos: React.FC<PhotosProps> = ({ OnSectionClick }) => {
    ...
    <div className="min-h-screen">
      ...
      <AnimatedSection>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {currentPhotos.map((photo, index) => (
            ...
            >
              <img src={photo.image.src} className="w-full h-48 object-cover" />
            </div>
          ))}
        </div>
      </AnimatedSection>
      ...
      {isModalOpen && selectedPhoto && (
        <BlogModal
          ...
          description={selectedPhoto.description || "No additional description provided."}
          image={selectedPhoto.image.src}
          onClose={closeModal}
        />
      )}
    </div>
